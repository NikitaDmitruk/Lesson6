...
public class Atm {
    ...
    public boolean Withdrawn(int amount) { //В общем тут написана попытка реализовать алгоритм динамического программирования, но не совсем получилось
        ...
        int[][] sums = new int[999999][2]; //Массив для возможных вариантов сумм
        sums[0][0] = 0; //Начальное значение
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < sizeSums; j++) {
                ...
                // Здесь определяем куда записать новую сумму. Если она не повторялась и если она меньше необходимой.
                ...
                //Здесь проверяем совпадение имеющейся суммы с необходимой, выводим результат и возвращаем булевое значение true
                if (sum == amount) {
